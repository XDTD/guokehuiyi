...
const _createActions = createActions('START_LOADING_ITEMS', 'CONTINUE_LOADING_INITIAL_ITEMS', 'FOUND_FIRST_NEW_ACTIVITY_DATE', 'GETTING_FUTURE_ITEMS', 'ALL_FUTURE_ITEMS_LOADED', 'ALL_PAST_ITEMS_LOADED', 'GOT_ITEMS_ERROR', 'START_LOADING_PAST_SAGA', 'START_LOADING_FUTURE_SAGA', 'START_LOADING_PAST_UNTIL_NEW_ACTIVITY_SAGA', 'START_LOADING_GRADES_SAGA', 'GOT_GRADES_SUCCESS', 'GOT_GRADES_ERROR', 'START_LOADING_PAST_UNTIL_TODAY_SAGA', 'PEEK_INTO_PAST_SAGA', 'PEEKED_INTO_PAST', 'GETTING_INIT_WEEK_ITEMS', 'GETTING_WEEK_ITEMS', 'START_LOADING_WEEK_SAGA', 'WEEK_LOADED', 'ALL_WEEK_ITEMS_LOADED', 'JUMP_TO_WEEK', 'JUMP_TO_THIS_WEEK', 'GOT_WAY_FUTURE_ITEM_DATE', 'GOT_WAY_PAST_ITEM_DATE', 'CLEAR_WEEKLY_ITEMS'),
      ...
      gettingInitWeekItems = _createActions.gettingInitWeekItems,
      gettingWeekItems = _createActions.gettingWeekItems,
      startLoadingWeekSaga = _createActions.startLoadingWeekSaga,
      weekLoaded = _createActions.weekLoaded,
      allWeekItemsLoaded = _createActions.allWeekItemsLoaded,
      jumpToWeek = _createActions.jumpToWeek,
      jumpToThisWeek = _createActions.jumpToThisWeek,
      gotWayPastItemDate = _createActions.gotWayPastItemDate,
      gotWayFutureItemDate = _createActions.gotWayFutureItemDate,
      clearWeeklyItems = _createActions.clearWeeklyItems;

export { startLoadingItems, continueLoadingInitialItems, foundFirstNewActivityDate, gettingFutureItems, allFutureItemsLoaded, allPastItemsLoaded, gotItemsError, startLoadingPastSaga, startLoadingFutureSaga, startLoadingPastUntilNewActivitySaga, startLoadingGradesSaga, gotGradesSuccess, gotGradesError, startLoadingPastUntilTodaySaga, peekIntoPastSaga, peekedIntoPast, gettingInitWeekItems, gettingWeekItems, startLoadingWeekSaga, weekLoaded, allWeekItemsLoaded, jumpToWeek, jumpToThisWeek, gotWayPastItemDate, gotWayFutureItemDate, clearWeeklyItems };
...
export function getWeeklyPlannerItems(fromMoment) {
  return (dispatch, getState) => {
    ...
    const waitingOnObserveeContextCodes = ((_getState$selectedObs = getState().selectedObservee) === null || _getState$selectedObs === void 0 ? void 0 : _getState$selectedObs.id) && !((_getState$selectedObs2 = getState().selectedObservee) !== null && _getState$selectedObs2 !== void 0 && _getState$selectedObs2.contextCodes);
...
function getWayFutureItem(fromMoment) {
  ...
  return (dispatch, getState) => {
    ...
    if (observed_user_id) {
      ...
      context_codes = state.singleCourse ? getContextCodesFromState(state) : void 0;
    }
...
function observedUserId(state) {
  var _state$selectedObserv;

  if ((_state$selectedObserv = state.selectedObservee) !== null && _state$selectedObserv !== void 0 && _state$selectedObserv.id && state.selectedObservee.id !== state.currentUser.id) {
