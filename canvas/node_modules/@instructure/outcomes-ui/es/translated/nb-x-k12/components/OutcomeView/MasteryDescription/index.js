...
var _percent_nbXK12_number = Intl.NumberFormat("nb-x-k12", { style: "percent" }).format,
    _default_nbXK12_number = Intl.NumberFormat("nb-x-k12", { style: "decimal" }).format,
    _dec,
    _class,
    _class2,
    _temp;
...
var scoringText = function scoringText(scoringMethod) {
  switch (scoringMethod.algorithm) {
    case 'decaying_average':
      return "Mestring beregnet etter gjennomsnittsresultat vektet etter siste resultater. De seneste resultatene teller som " + _percent_nbXK12_number(scoringMethod.algorithm_data.decaying_average_percent) + " av mestringsgrad, gjennomsnittet av alle andre resultater teller som " + _percent_nbXK12_number(1.0 - scoringMethod.algorithm_data.decaying_average_percent) + " av mestringsgraden. Dersom det er kun ett resultat, vil det enkelte resultatet bli returnert.";

    case 'n_mastery':
      return "Mestring beregnet etter n antall ganger. M\xE5 oppn\xE5 mestring minst " + _default_nbXK12_number(scoringMethod.algorithm_data.n_mastery_count) + " ganger.  Resultater over mestringsniv\xE5 blir jevnet med snittet for \xE5 beregne sluttresultatet.";

    case 'highest':
      return "Mestring beregnet etter h\xF8yest resultat. Mestringsresultat viser den h\xF8yeste resultatet for en vurdert vurdering.";

    case 'latest':
      return "Mestring beregnet etter seneste resultat. Mestringsresultat viser vurderingen som er vurdert sist.";
...
var masteryText = function masteryText(masteryPercent, artifactTypeName) {
  return "Ved \xE5 justere til dette " + artifactTypeName + " hvis eleven scorer h\xF8yere enn " + _percent_nbXK12_number(masteryPercent) + " vil mestring oppn\xE5s.";
};
