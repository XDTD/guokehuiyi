...
var SearchResults = /*#__PURE__*/function (_React$Component) {
  ...
  _createClass(SearchResults, [{
    ...
    value: function componentDidUpdate(prevProps) {
      ...
      if (screenreaderNotification) {
        if (!isSearchLoading && prevProps.isSearchLoading) {
          screenreaderNotification("Rezultati za stran " + searchPage + " od " + this.getPageCount() + " so posodobljeni.");
          screenreaderNotification(this.getResultCountText());
        }

        if (!isEqual(searchPage, prevProps.searchPage)) {
          screenreaderNotification("Nalagam stran " + searchPage);
        }
      }
    }
  }, {
    ...
    value: function getResultCountText() {
      ...
      var searchTotal = this.props.searchTotal;
      return _s = searchTotal, _n = +_s, _n === 0 ? "Iskanje je/ni vrnilo rezultatov " : (_v = (_s + '.').split('.')[1].length, _i = Math.floor(Math.abs(+_s)), _v === 0 && _i % 100 === 1 ? _default_sl_number(searchTotal) + " rezultat " : _default_sl_number(searchTotal) + " rezultate");
    }
  }, {
    key: "renderResults",
    ...
    value: function renderLoading() {
      return React.createElement(Flex, {
        ...
        renderTitle: "Nalaganje rezultatov iskanja"
      })));
    }
  }, {
    key: "render",
