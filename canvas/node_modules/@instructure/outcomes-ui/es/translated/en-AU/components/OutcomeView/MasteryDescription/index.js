...
var _percent_enAU_number = Intl.NumberFormat("en-AU", { style: "percent" }).format,
    _default_enAU_number = Intl.NumberFormat("en-AU", { style: "decimal" }).format,
    _dec,
    _class,
    _class2,
    _temp;
...
var scoringText = function scoringText(scoringMethod) {
  switch (scoringMethod.algorithm) {
    case 'decaying_average':
      return "Mastery calculated by Decaying Average. Most recent counts as " + _percent_enAU_number(scoringMethod.algorithm_data.decaying_average_percent) + " of mastery weight, average of all other results count as " + _percent_enAU_number(1.0 - scoringMethod.algorithm_data.decaying_average_percent) + " of weight. If there is only one result, the single score will be returned.";

    case 'n_mastery':
      return "Mastery calculated by n Number of Times. Must achieve mastery at least " + _default_enAU_number(scoringMethod.algorithm_data.n_mastery_count) + " times.  Scores above mastery will be averaged to calculate final score.";
...
var masteryText = function masteryText(masteryPercent, artifactTypeName) {
  return "By aligning to this " + artifactTypeName + " if the student scores above " + _percent_enAU_number(masteryPercent) + " mastery will be achieved.";
};
