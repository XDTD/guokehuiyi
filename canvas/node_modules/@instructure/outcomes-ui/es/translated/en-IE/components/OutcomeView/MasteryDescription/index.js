...
var _percent_enIE_number = Intl.NumberFormat("en-IE", { style: "percent" }).format,
    _default_enIE_number = Intl.NumberFormat("en-IE", { style: "decimal" }).format,
    _dec,
    _class,
    _class2,
    _temp;
...
var scoringText = function scoringText(scoringMethod) {
  switch (scoringMethod.algorithm) {
    case 'decaying_average':
      return "Mastery calculated by decaying average. Most recent counts as " + _percent_enIE_number(scoringMethod.algorithm_data.decaying_average_percent) + " of mastery weight, average of all other results count as " + _percent_enIE_number(1.0 - scoringMethod.algorithm_data.decaying_average_percent) + " of weight. If there is only one result, the single score will be returned.";

    case 'n_mastery':
      return "Mastery calculated by number of times. Must achieve mastery at least " + _default_enIE_number(scoringMethod.algorithm_data.n_mastery_count) + " times.  Scores above mastery will be averaged to calculate final score.";

    case 'highest':
      return "Mastery calculated using Highest Score. Mastery score reflects the highest score of a graded assessment.";
...
var masteryText = function masteryText(masteryPercent, artifactTypeName) {
  return "By aligning to this " + artifactTypeName + " if the student scores above " + _percent_enIE_number(masteryPercent) + " mastery will be achieved.";
};
