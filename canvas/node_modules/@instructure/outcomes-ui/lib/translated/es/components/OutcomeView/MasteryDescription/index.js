...
var _percent_es_number = Intl.NumberFormat("es", { style: "percent" }).format,
    _default_es_number = Intl.NumberFormat("es", { style: "decimal" }).format,
    _dec,
    _class,
    _class2,
    _temp;
...
var scoringText = function scoringText(scoringMethod) {
  switch (scoringMethod.algorithm) {
    case 'decaying_average':
      return "Dominio calculado, seg\xFAn el promedio de decaimiento. El resultado m\xE1s reciente se considera como el " + _percent_es_number(scoringMethod.algorithm_data.decaying_average_percent) + " de la ponderaci\xF3n del dominio; el promedio de todos los dem\xE1s resultados se considera como el " + _percent_es_number(1.0 - scoringMethod.algorithm_data.decaying_average_percent) + " de la ponderaci\xF3n. Si solo hay un resultado, se mostrar\xE1 un \xFAnico puntaje.";

    case 'n_mastery':
      return "Dominio calculado, seg\xFAn n n\xFAmero de veces. Debe alcanzar dominio al menos " + _default_es_number(scoringMethod.algorithm_data.n_mastery_count) + " veces.  Los puntajes que superan el dominio se promediar\xE1n para calcular el puntaje final.";

    case 'highest':
      return "Dominio calculado, seg\xFAn el puntaje m\xE1ximo. El puntaje del dominio refleja el puntaje m\xE1ximo de una evaluaci\xF3n calificada.";

    case 'latest':
      return "Dominio calculado seg\xFAn el puntaje m\xE1s reciente. El puntaje del dominio refleja la evaluaci\xF3n calificada m\xE1s recientemente.";
...
var masteryText = function masteryText(masteryPercent, artifactTypeName) {
  return "Al alinearlo con este " + artifactTypeName + " si el estudiante obtiene un puntaje superior a " + _percent_es_number(masteryPercent) + ", alcanzar\xE1 el dominio.";
};
