...
var _percent_enAUXUnimelb_number = Intl.NumberFormat("en-AU-x-unimelb", { style: "percent" }).format,
    _default_enAUXUnimelb_number = Intl.NumberFormat("en-AU-x-unimelb", { style: "decimal" }).format,
    _dec,
    _class,
    _class2,
    _temp;
...
var scoringText = function scoringText(scoringMethod) {
  switch (scoringMethod.algorithm) {
    case 'decaying_average':
      return "Mastery calculated by Decaying Average. Most recent counts as " + _percent_enAUXUnimelb_number(scoringMethod.algorithm_data.decaying_average_percent) + " of mastery weight, average of all other results count as " + _percent_enAUXUnimelb_number(1.0 - scoringMethod.algorithm_data.decaying_average_percent) + " of weight. If there is only one result, the single score will be returned.";

    case 'n_mastery':
      return "Mastery calculated by n Number of Times. Must achieve mastery at least " + _default_enAUXUnimelb_number(scoringMethod.algorithm_data.n_mastery_count) + " times.  Scores above mastery will be averaged to calculate final score.";

    case 'highest':
      return "Mastery calculated by Highest Score. Mastery score reflects the highest score of a graded assessment.";

    case 'latest':
      return "Mastery calculated by Most Recent Score. Mastery score reflects the most recent graded assessment.";
...
var masteryText = function masteryText(masteryPercent, artifactTypeName) {
  return "By aligning to this " + artifactTypeName + " if the student scores above " + _percent_enAUXUnimelb_number(masteryPercent) + " mastery will be achieved.";
};
