...
var _percent_nl_number = Intl.NumberFormat("nl", { style: "percent" }).format,
    _default_nl_number = Intl.NumberFormat("nl", { style: "decimal" }).format,
    _dec,
    _class,
    _class2,
    _temp;
...
var scoringText = function scoringText(scoringMethod) {
  switch (scoringMethod.algorithm) {
    case 'decaying_average':
      return "Bekwaamheid berekend op basis van Achteruitgaand gemiddelde. Meest recente tellingen als " + _percent_nl_number(scoringMethod.algorithm_data.decaying_average_percent) + " van gewicht voor bekwaamheid, gemiddelde van alle andere resultaten telt als " + _percent_nl_number(1.0 - scoringMethod.algorithm_data.decaying_average_percent) + " van gewicht. Als er maar \xE9\xE9n resultaat is, wordt deze ene score teruggegeven.";

    case 'n_mastery':
      return "Bekwaamheid berekend op basis van n aantal keren. Moet bekwaamheid minimaal " + _default_nl_number(scoringMethod.algorithm_data.n_mastery_count) + " keer behalen.  Scores hoger dan bekwaamheid worden gemiddeld om de eindscore te berekenen.";

    case 'highest':
      return "Bekwaamheid berekend op basis van Hoogste score. Bekwaamheidsscore geeft de hoogste score weer van een beoordeelde opdracht.";

    case 'latest':
      return "Bekwaamheid berekend op basis van Meest recente score. Bekwaamheidsscore geeft de meest recente score weer van een beoordeelde opdracht.";
...
var masteryText = function masteryText(masteryPercent, artifactTypeName) {
  return "Door af te stemmen op deze " + artifactTypeName + " als de cursist hoger scoort dan " + _percent_nl_number(masteryPercent) + ", is bekwaamheid behaald.";
};
