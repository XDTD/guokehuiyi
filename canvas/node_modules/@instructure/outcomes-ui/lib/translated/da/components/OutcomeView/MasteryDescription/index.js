...
var _percent_da_number = Intl.NumberFormat("da", { style: "percent" }).format,
    _default_da_number = Intl.NumberFormat("da", { style: "decimal" }).format,
    _dec,
    _class,
    _class2,
    _temp;
...
var scoringText = function scoringText(scoringMethod) {
  switch (scoringMethod.algorithm) {
    case 'decaying_average':
      return "Beherskelse beregnet med v\xE6gtet gennemsnit (Decaying Average). Seneste t\xE6ller som " + _percent_da_number(scoringMethod.algorithm_data.decaying_average_percent) + " af beherskelsesv\xE6gtning, gennemsnit af alle andre resultater t\xE6ller som " + _percent_da_number(1.0 - scoringMethod.algorithm_data.decaying_average_percent) + " af v\xE6gtning. Hvis der kun er \xE9t resultat, bliver det enkle resultat returneret.";

    case 'n_mastery':
      return "Beherskelse beregnes ved n antal gange. Skal opn\xE5 beherskelse mindst " + _default_da_number(scoringMethod.algorithm_data.n_mastery_count) + " gange.  Der udregnes gennemsnit for resultat over beherskelse for at beregne slutresultatet.";

    case 'highest':
      return "Beherskelse beregnes pr. h\xF8jeste score. Beherskelsesresultat afspejler det h\xF8jeste resultat for en bed\xF8mt vurdering.";

    case 'latest':
      return "Beherskelse beregnes pr. seneste resultat. Beherskelsesresultat afspejler den senest bed\xF8mte vurdering.";
...
var masteryText = function masteryText(masteryPercent, artifactTypeName) {
  return "Ved at tilpasse sig dette " + artifactTypeName + ", hvis den studerende f\xE5r over " + _percent_da_number(masteryPercent) + ", opn\xE5s beherskelse.";
};
